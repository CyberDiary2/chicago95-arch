#!/usr/bin/env bash
# chicago95-arch -- automated rice + pentest setup for Arch Linux XFCE
...
# install packages one at a time so a missing name skips rather than aborts
pkg() {
    for p in "$@"; do
...
# ── main ──────────────────────────────────────────────────────────────────────
main() {
    log "chicago95-arch setup starting"
